...
import { treasuryHistory } from "./mock-data"
...
interface GroupTreasuryChartProps {
	period: DashboardPeriod
	className?: string
}

export function GroupTreasuryChart({ period, className }: GroupTreasuryChartProps) {
	const monthsToShow = PERIOD_MONTHS[period]
	const data = treasuryHistory.slice(-monthsToShow)

	return (
		<div
			className={cn(
				"flex min-h-72 flex-col justify-between rounded-2xl border border-border/60 bg-card p-6 shadow-sm",
				className,
			)}
		>
			...
			<div className="h-64 w-full">
				<ResponsiveContainer width="100%" height="100%">
					<AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
						<defs>
							<linearGradient id="gradientTreasury" x1="0" y1="0" x2="0" y2="1">
								<stop offset="5%" stopColor="var(--chart-1)" stopOpacity={0.25} />
								<stop offset="95%" stopColor="var(--chart-1)" stopOpacity={0} />
							</linearGradient>
							<linearGradient id="gradientTreasuryValue" x1="0" y1="0" x2="0" y2="1">
						...
						<Area
							...
							strokeWidth={2.5}
							fillOpacity={1}
							fill="url(#gradientTreasuryValue)"
							dot={false}
							activeDot={{ r: 4, strokeWidth: 2, stroke: "var(--card)" }}
						/>
					</AreaChart>
			...
			<div className="flex items-center justify-center gap-5">
				<ChartLegendItem color="var(--chart-1)" label="Trésorerie" />
