...
7. [x] Add verification flow: `POST /auth/verify`, `POST /auth/resend-verification`, satisfies **AC-10**. (2026-07-19 fix: `AuthService.register` never actually called `VerificationService.requestVerification`, so no verification email was ever sent on signup; wired it in, and gave the web app's verify banner its own resend button since it had no visible path to trigger a resend before.)
