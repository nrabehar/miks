...
- **AC-3**: When a project reaches `APPROVED`, the system attempts activation in the same transaction: if sourceVault has enough balance for requestedBudget, it debits sourceVault and credits the project's payoutVault (if one was designated) in one `PROJECT_PAYOUT` transaction, and the project moves to `ACTIVE`. Vote resolution and activation are guarded two ways in the same transaction: a status conditional update on the project row (so two concurrent lazy resolutions of the same vote cannot both execute the payout), and a balance conditional update on the debit itself (so two different projects racing to draw down the same shared sourceVault at the same moment cannot both succeed past its balance).
...
- **AC-8**: Any active member can close an `ACTIVE` project at any time. Closing it auto liquidates whatever balance remains in each project vault per that vault's applicable active flow rule (or, if none applies, leaves the balance exactly as is, an orphaned nonzero balance the group can see and later address, never silently zeroed out), then moves the project to `CLOSED`. Closure is guarded by the same status conditional update pattern as vote resolution/activation (only proceeds while the project row still reads `ACTIVE`), so two concurrent close calls, or a close racing a concurrent revenue/expense declaration, cannot both act on the same stale balance.
...
- The `PROJECT_PAYOUT` debit against sourceVault is itself a balance conditional update (only succeeds while sourceVault's balance still covers requestedBudget at the moment of the write), independent of the project status guard above. This matters because sourceVault is a `GROUP` vault a project does not own alone: two different projects approved around the same moment could both read it as sufficiently funded, so the status guard on the project row is not enough by itself to stop a double debit that overdraws it.
- Project closure (AC-8) uses the same status conditional update pattern (only proceeds while the project row still reads `ACTIVE`), so a double close call, or a close racing a concurrent revenue/expense declaration on the same project, cannot both liquidate against the same stale vault balance.
...
8. Project closure: guarded by a status conditional update (only proceeds while the project still reads `ACTIVE`), liquidate remaining project vault balances per each vault's active flow rule (or leave the balance exactly as is if none applies, never zeroed out), move the project to `CLOSED`, satisfies **AC-8**
